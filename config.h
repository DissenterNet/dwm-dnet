...
/* -------------------------
 * Mode & trigger compatibility helpers
 * -------------------------
 *
 * dwm.c provides the canonical definitions for key/button modes and the KeyTrigger
 * enum. Config should NOT re-declare those enums (they collide with the macros/typedefs
 * in dwm.c and cause preprocessor / enum redefinition errors).
 *
 * This section provides small compatibility fallbacks for configs that expect
 * KEYMODE_DEFAULT or BMODE_DEFAULT macros, but otherwise defers to the
 * definitions in dwm.c.
 */

/* KEYMODE_ANY is expected to be defined by dwm.c before config.h is included.
 * If it isn't, provide a safe fallback (rare).
 */
#ifndef KEYMODE_ANY
#define KEYMODE_ANY -1
#endif

/* Provide a default keymode symbol the config can use when it previously used
 * KEYMODE_DEFAULT. Mapping it to KEYMODE_ANY would make every key global; mapping
 * it to KEYMODE_NORMAL keeps current behavior. Use NORMAL for backwards compatibility.
 */
#ifndef KEYMODE_DEFAULT
#define KEYMODE_DEFAULT 0
#endif

/* Provide a default button-mode symbol for config convenience */
#ifndef BMODE_DEFAULT
#define BMODE_DEFAULT 0
#endif


/* chord timeout (ms) — dwm.c defines chordtimeoutms; set a safe default here if not set */
#ifndef chordtimeoutms
static int chordtimeoutms = 800;
#endif

/* provide a fallback empty keychords array so sizeof(keychords) works if no chords are defined yet.
 * Real chords should be created in chords/ and then provide a compile-time array of pointers:
 *   static Keychord *keychords[] = { &kc1, &kc2, NULL };
 */
/*#ifndef KEYCHORDS_FALLBACK*/
/*static Keychord *keychords[] = { NULL };*/
/*#endif*/
...
/* helper functions that rely on dwm symbols */
/*#include "helpers/helper.h"*/
#include "utils/helpers/helpers_vim.h"


/* keymaps (per-mode) */
#include "utils/keys/key.h"
/*#include "keys/keymode-default.h"*/
/*#include "keys/keymode-normal.h"*/
/*#include "keys/keymode-test.h"*/
...
/*#include "buttons/bmode-alt.h"*/
/*#include "buttons/bmode-default.h"*/
/*#include "buttons/bmode-normal.h"*/
/*#include "buttons/bmode-resize.h"*/

/* /chords/ */
#include "utils/chords/chords_decl.h"
#include "utils/chords/chord.h"
...
#ifndef KX
#define KX(MOD, KEY, ACTION, TAG) { (MOD), (KEY), (ACTION), (TAG), KEYMODE_DEFAULT, KeyPressOnly }
#endif
...
static const Key keys[] = {
	...
	{ MODKEY|ShiftMask,             XK_v,      setkeymode,     {.i = KEYMODE_VIM} }, /* Switch to vim mode */
...
#ifndef B
/* Button initializer macro: click, mask, button, func, arg, bmode */
#define B(click, mask, button, func, arg) { (click), (mask), (button), (func), (arg), BMODE_DEFAULT }
#endif
